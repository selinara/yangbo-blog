    ...
    public static String doGet(String url,  String charset) throws Exception {
        ...
        try {
            ...
            if (responseCode != HttpStatus.SC_OK) {
                logger.info("responseCode=" + responseCode + ", url=" + url );
                return ERROR_HTTP_RESPONSE_CODE;
            }
            return EntityUtils.toString(response.getEntity());
        } catch (SocketTimeoutException ex) {
            logger.error("http get occurs timeout error, url=" + url  , ex);
            return ERROR_HTTP_TIMEOUT_RESPONSE_CODE;
        } catch (Exception e) {
            logger.error("http get occurs exception error, url=" + url , e);
            return ERROR_HTTP_RESPONSE_CODE;
        } finally {
            if (response != null) {
